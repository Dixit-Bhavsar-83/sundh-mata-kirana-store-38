...
async function confirmReset() {
    const isSure = confirm("⚠️ WARNING: Boss, kya aap sure hain? Ye button press karte hi saari orders, customers aur history permanently delete ho jayegi!");
    
    if (isSure) {
        // Supabase ke us function ko call karein
        const { data, error } = await supabase.rpc('reset_store_data');
        
        if (!error) {
            alert("App puri tarah fresh ho gayi hai! 🚀");
            localStorage.clear(); // Local cache bhi saaf
            window.location.href = '../login.html'; // Login par bhejo
        } else {
            alert("Error: Data saaf nahi ho paya. " + error.message);
        }
    }
}