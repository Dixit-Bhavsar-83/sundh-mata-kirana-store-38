...
async function acceptOrder(event, orderId) {
    // Restriction hata di gayi hai - Ab multiple orders accept ho sakte hain
    
    // Optional: Agar button click par loading dikhana hai toh:
    const btn = event?.currentTarget;
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<i class="fa fa-spinner spinning mr-1"></i>Accepting...';
    }
    ...
    try {
        ...
        if (res.status === 'success') {
            ...
            await loadMine(); // List refresh karo
        } else {
            alert("Order accept karne mein error aaya.");
        }
    } catch (e) { 
        console.error(e); 
    } finally {
